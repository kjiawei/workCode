...
void *routine(void *arg)
{
	...
	while(1)
	{
		...
		(p->task)(p->pathname,p->test);											//运行任务	4
...
bool add_task(thread_pool *pool,
			void *(*task)(const char *pathname, const char *test), const char *pathname, const char *test)		//3
{
	...
	new_task->pathname = pathname;						//3
	new_task->test = test;								//3
	new_task->next = NULL;
