...
int rec_read(char *pathname,list dir,list reg)//递归读取目录
{
	int count = 0;
	int count1 = 0;
	int count2 = 0;
...
int dir_judge(char *arg1,char *arg2,list dir,list reg)//目录判断及读取
{
	int count = 0;
	int count1 = 0;
	int count2 = 0;
	...
	else
	{
		while(1)
		{
			...
			if(ep->d_type == DT_DIR && ep->d_name[0] != '.')//判断是否为目录且不是当前路径.和父路径..
			{
				...
				list_add(dir,buf);//先添加到目录链表，确保不会发生嵌套创建目录
				count1 = rec_read(buf,dir,reg);//循环调用目录读取函数
				count2 = count2+count1;
			}
...
void *copy_reg(void *arg)//复制普通文件功能函数
{
	...
	if(p->reg != NULL)
	{
		int fd1 = open(p->reg->filename,O_RDONLY);
		if(fd1 == -1)
		{
			printf("%s open failed:",p->reg->filename);
			perror("open()\n");
			exit(1);
		}
		...
		for(i = 0;i<strlen(p->reg->filename);i++)
		{
			if(p->reg->filename[i] == '/')
				j = i;
		}
		for(i = 0;i<strlen(p->reg->filename)-j;i++)
		{
			tmp3[i] = p->reg->filename[j+i];
		}
		for(i = 0;i<j-strlen(p->sourcepath);i++)
		{
			tmp1[i] = p->reg->filename[i+strlen(p->sourcepath)];
		}
		...
		while(1)
		{
			...
			//判断是否有重名文件
			if(strcmp((p->reg->filename)+j+1,ep->d_name) == 0)
			{
				flag = 1;
