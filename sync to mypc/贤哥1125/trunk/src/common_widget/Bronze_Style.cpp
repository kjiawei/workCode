...
int BronzeStyle::styleHint(StyleHint which,
...
QIcon BronzeStyle::standardIconImplementation(StandardPixmap which,
                                              ...
                                              const QWidget *widget) const
{
    QImage image = QWindowsStyle::standardPixmap(which,option,widget).toImage();
                                  ...
                                  const QWidget *widget) const
{
    if (whichControl == CC_SpinBox)
    {
        ...
        switch(whichSubControl)
        {
            ...
            case SC_SpinBoxDown:
                return visualRect(option->direction,
                                  option->rect,
                                  QRect(option->rect.x(),option->rect.y(),buttonWidth,option->rect.height()));

            case SC_SpinBoxUp:
                return visualRect(option->direction,
                                  option->rect,
                                  QRect(option->rect.right()-buttonWidth,option->rect.y(),buttonWidth,option->rect.height()));
...
void BronzeStyle::drawBronzeBevel(const QStyleOption *option, QPainter *painter) const
{
    ...
    gradient.setColorAt(0.2,buttonColor.light(coeff));
