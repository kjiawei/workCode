...
#ifndef SCENE_SETUP_CURVE_NA_H
#define SCENE_SETUP_CURVE_NA_H
...
class SceneSetupCurveNa;

extern bool naEnableEdit; //预定义曲线是否可编辑 true 可以 false 不可以#/
extern char select_nacure_num;   //从外部进来该页面时，选择的曲线#/

/*-------------------------柱状图类-------------------------------*/
//每个治疗时段的柱状图类#/
class ColumnarCurve: public QLabel
{
    Q_OBJECT

public:
    ColumnarCurve(QWidget *parent,SceneSetupCurveNa *p,int x,int y,int order);    
...
private:
    ...
    int yAxis;//y坐标#/
    bool passMark; //点击标记#/
    SceneSetupCurveNa *topscene; //为了操控上层页面的控件而定义#/
};

/*-------------------------钠曲线显示编辑页面类-------------------------------*/
//钠曲线显示编辑页面类#/
class SceneSetupCurveNa:public QObject, public ActionCustom,public PieceVisible,public LangSetText
{
    Q_OBJECT
public:
    SceneSetupCurveNa(QWidget *parent=0); //构造函数#/
    ~SceneSetupCurveNa();
...
public:
    ...
    ColumnarCurve *curveArray[10]; //指向柱状图的指针，默认数量是10个#/
    //各种文本标签#/
