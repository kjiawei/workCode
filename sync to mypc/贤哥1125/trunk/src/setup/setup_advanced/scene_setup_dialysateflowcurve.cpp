...
/*-------------------------柱状图类----------------------------------*/
//界面设置函数#/
inline void ColumDiaCurve::setUi()
{
...
//构造函数#/
ColumDiaCurve::ColumDiaCurve(QWidget *parent,SceneSetupDialysateFlowCurve *p,int x,int y,int order):
...
void ColumDiaCurve::mousePressEvent(QMouseEvent *ev)
{
    if(selectMark==0)
        return;
    ...
    naValue=usr_params.Params_Dialyser.Rate.attribute.max_value-((ev->y()-3)*((usr_params.Params_Dialyser.Rate.attribute.max_value-usr_params.Params_Dialyser.Rate.attribute.min_value)/(SCENE_SETUP_CURVE_STRIP_HIGH-6.0)));
...
//鼠标移动事件#/
void ColumDiaCurve::mouseMoveEvent(QMouseEvent *ev)
{
    if(passMark==false)
        return;
...
//鼠标释放事件#/
void ColumDiaCurve::mouseReleaseEvent(QMouseEvent *ev)
{
    passMark=false;
    ev->ignore();
...
//焦点进入#/
void ColumDiaCurve::focusInEvent(QFocusEvent *ev)
{
...
//焦点移出#/
void ColumDiaCurve::focusOutEvent(QFocusEvent *ev)
{
    ev=ev;
    selectMark=0;
    this->setStyleSheet("border-width: 3px; border-style: outset; \
...
//重绘事件#/
void ColumDiaCurve::paintEvent(QPaintEvent *)
{
    QPainter paint(this);

    naValue=naValue>usr_params.Params_Dialyser.Rate.attribute.max_value?usr_params.Params_Dialyser.Rate.attribute.max_value:naValue;
    naValue=naValue<usr_params.Params_Dialyser.Rate.attribute.min_value?usr_params.Params_Dialyser.Rate.attribute.min_value:naValue;
    float high=((usr_params.Params_Dialyser.Rate.attribute.max_value-naValue)*1.0/(usr_params.Params_Dialyser.Rate.attribute.max_value-usr_params.Params_Dialyser.Rate.attribute.min_value))*(SCENE_SETUP_CURVE_STRIP_HIGH-6.0);
    ...
    labeltop.setText(QString().setNum(naValue,'f',0));
...
//  qDebug()<<"paintEvent";
    ...
    //显示#/
void ColumDiaCurve::setShow(bool vis)
{
    this->setVisible(vis);
...
/*-------------------------曲线页面显示类-------------------------------*/
//界面设置函数#/
inline void SceneSetupDialysateFlowCurve::setUi()
{
...
//构造函数#/
SceneSetupDialysateFlowCurve::SceneSetupDialysateFlowCurve(QWidget *parent):
QObject(parent),
...
//                      0.01,"Na Value:","Na",false,INT);
      ...
      ic->appendWidget(&lineEditSetNum,NumInputMethod,LineEdit,NULL,500,1000,
                        0.1,"Dia Value:","Dialysate",false,INT);
 ...
 //   nowSelectCurve=0;
    for(int i=0;i<10;i++)
    {
        curveArray[i]=new ColumDiaCurve(parent,this,SCENE_SETUP_CURVE_START_X+52*i,
                                        SCENE_SETUP_CURVE_START_Y,i);
        curveArray[i]->naValue=700;
...
SceneSetupDialysateFlowCurve::~SceneSetupDialysateFlowCurve()
{
    for(int i=0;i<10;i++)
    {
...
//界面文字显示设置#/
void SceneSetupDialysateFlowCurve::CustomSetText()
{

    labelTitleTop.setText(DE_TR_DIAFLOWPROFILE);
...
//    labelYaxisUnit.setText("[ms/cm]");
    labelYaxisUnit.setText("ml/min");


    label160.setText(QString::number(usr_params.Params_Dialyser.Rate.attribute.max_value)+"-");
    label151.setText(QString::number(((15.1-12)/(16-12))*(usr_params.Params_Dialyser.Rate.attribute.max_value-usr_params.Params_Dialyser.Rate.attribute.min_value)+usr_params.Params_Dialyser.Rate.attribute.min_value,'f',0)+"-");
    label142.setText(QString::number(((14.2-12)/(16-12))*(usr_params.Params_Dialyser.Rate.attribute.max_value-usr_params.Params_Dialyser.Rate.attribute.min_value)+usr_params.Params_Dialyser.Rate.attribute.min_value,'f',0)+"-");
    label134.setText(QString::number(((13.4-12)/(16-12))*(usr_params.Params_Dialyser.Rate.attribute.max_value-usr_params.Params_Dialyser.Rate.attribute.min_value)+usr_params.Params_Dialyser.Rate.attribute.min_value,'f',0)+"-");
    label125.setText(QString::number(((12.5-12)/(16-12))*(usr_params.Params_Dialyser.Rate.attribute.max_value-usr_params.Params_Dialyser.Rate.attribute.min_value)+usr_params.Params_Dialyser.Rate.attribute.min_value,'f',0)+"-");
...
//页面显示调用函数--#/
void SceneSetupDialysateFlowCurve::setCusVisible(bool vis, char *para)
{
    para=para;

#if SYSTEM_DEBUG_FLAG
    Debug_insert("SceneSetupDialysateFlowCurve::setCusVisible-in");
#endif
...
#if SYSTEM_DEBUG_FLAG
    Debug_insert("SceneSetupDialysateFlowCurve::setCusVisible-out");
#endif
}

void SceneSetupDialysateFlowCurve::ActionHandle(const void* dataPtr, int name)
{
#if SYSTEM_DEBUG_FLAG
    Debug_insert("SceneSetupDialysateFlowCurve-ActionHandle-in");
#endif
...
#if SYSTEM_DEBUG_FLAG
    Debug_insert("SceneSetupDialysateFlowCurve-ActionHandle-out");
#endif
}

//根据选择的曲线号，读取对应的数据文件显示曲线#/
void SceneSetupDialysateFlowCurve::selectedCurve(int num)
{
    num=num;
...
//编辑框选择时#/
void SceneSetupDialysateFlowCurve::lineEditInFocus()
{
...
//全加按钮点击时#/
void SceneSetupDialysateFlowCurve::allAddClicked()
{
...
//全减按钮点击#/
void SceneSetupDialysateFlowCurve::allSubClicked()
{
...
//曲线选择按钮---+#/
void SceneSetupDialysateFlowCurve::selectAddClicked()
{
    nowSelectCurve++;
...
void SceneSetupDialysateFlowCurve::selectSubClicked()
{
    nowSelectCurve--;
    if(nowSelectCurve<0)
        nowSelectCurve=15;
...
void SceneSetupDialysateFlowCurve::buttonSaveClicked()//保存按钮#/
{
...
void SceneSetupDialysateFlowCurve::buttonOkClicked()
{
    QString str;
    int i=0;

    if((residue_secCount<3600)&&(G_DIA_PROFILE_FLAG==false))//小于60分钟不给应用#/
    {
    ...
    if(G_DIA_PROFILE_FLAG==true)  //该曲线正在应用,是否取消#/
    {
        //确定是否应用#/
        ...
        else
        {
            ...
            packet.Wspeed = G_DIA_PROFILE_RELVULE;
    ...
    else if(G_DIA_PROFILE_FLAG==false) //该曲线未应用,是否应用#/
    {
        //确定是否应用#/
        ...
        G_DIA_PROFILE_FLAG=true;

        //曲线应用处理#/
        for(i=0;i<10;i++)
        {
            A_DIA_PROFILE[i].curevalue=curveArray[i]->naValue;
            A_DIA_PROFILE[i].flag=false;
        }
        G_DIA_PROFILE_NUM=-1;      //曲线当前运行第几段#/
        G_DIA_PROFILE_START=G_SYS_CURTIME;  //曲线开始时间#/
        G_DIA_PROFILE_END=G_SYS_CURTIME+residue_secCount;    //曲线结束时间#/
        ...
        G_DIA_PROFILE_RELVULE=packet.Wspeed;    //曲线运行前的设置值#/
    }
    ...
    logInfo log;
    //log.logId=1;
    log.opration="DiaFlow profile Run";
...
//取消#/
void SceneSetupDialysateFlowCurve::buttonCancelClicked()
{
