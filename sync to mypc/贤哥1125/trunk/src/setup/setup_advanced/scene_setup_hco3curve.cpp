...
#define DE_HCO3_SETMAX  40
#define DE_HCO3_SETMIN  20
...
/*-------------------------柱状图类----------------------------------*/
//界面设置函数#/
inline void ColumHco3Curve::setUi()
{
...
//构造函数#/
ColumHco3Curve::ColumHco3Curve(QWidget *parent,SceneSetupHco3Curve *p,int x,int y,int order):
...
void ColumHco3Curve::mousePressEvent(QMouseEvent *ev)
{
    if(selectMark==0)
        return;
    ...
    naValue=DE_HCO3_SETMAX-((ev->y()-3)*((DE_HCO3_SETMAX-DE_HCO3_SETMIN)/(SCENE_SETUP_CURVE_STRIP_HIGH-6.0)));
...
//鼠标移动事件#/
void ColumHco3Curve::mouseMoveEvent(QMouseEvent *ev)
{
    if(passMark==false)
        return;
...
//鼠标释放事件#/
void ColumHco3Curve::mouseReleaseEvent(QMouseEvent *ev)
{
    passMark=false;
    ev->ignore();
...
//焦点进入#/
void ColumHco3Curve::focusInEvent(QFocusEvent *ev)
{
...
//焦点移出#/
void ColumHco3Curve::focusOutEvent(QFocusEvent *ev)
{
    ev=ev;
    selectMark=0;
    this->setStyleSheet("border-width: 3px; border-style: outset; \
...
//重绘事件#/
void ColumHco3Curve::paintEvent(QPaintEvent *)
{
    QPainter paint(this);

    naValue=naValue>DE_HCO3_SETMAX?DE_HCO3_SETMAX:naValue;
    naValue=naValue<DE_HCO3_SETMIN?DE_HCO3_SETMIN:naValue;
    float high=((DE_HCO3_SETMAX-naValue)*1.0/(DE_HCO3_SETMAX-DE_HCO3_SETMIN))*(SCENE_SETUP_CURVE_STRIP_HIGH-6.0);
...
//  qDebug()<<"paintEvent";
    ...
    //显示#/
void ColumHco3Curve::setShow(bool vis)
{
    this->setVisible(vis);
...
/*-------------------------曲线页面显示类-------------------------------*/
//界面设置函数#/
inline void SceneSetupHco3Curve::setUi()
{
...
//构造函数#/
SceneSetupHco3Curve::SceneSetupHco3Curve(QWidget *parent):
QObject(parent),
...
//                      0.01,"Na Value:","Na",false,INT);
      ...
      ic->appendWidget(&lineEditSetNum,NumInputMethod,LineEdit,NULL,DE_HCO3_SETMIN,DE_HCO3_SETMAX,
 ...
 //   nowSelectCurve=0;
    for(int i=0;i<10;i++)
    {
        curveArray[i]=new ColumHco3Curve(parent,this,SCENE_SETUP_CURVE_START_X+52*i,
                                        SCENE_SETUP_CURVE_START_Y,i);
        curveArray[i]->naValue=32;
...
SceneSetupHco3Curve::~SceneSetupHco3Curve()
{
    for(int i=0;i<10;i++)
    {
...
//界面文字显示设置#/
void SceneSetupHco3Curve::CustomSetText()
{

    labelTitleTop.setText(DE_TR_HCO3PROFILE);
...
//    labelYaxisUnit.setText("[ms/cm]");
    ...
    label160.setText(QString::number(DE_HCO3_SETMAX)+"-");
    label151.setText(QString::number(((15.1-12)/(16-12))*(DE_HCO3_SETMAX-DE_HCO3_SETMIN)+DE_HCO3_SETMIN,'f',0)+"-");
    label142.setText(QString::number(((14.2-12)/(16-12))*(DE_HCO3_SETMAX-DE_HCO3_SETMIN)+DE_HCO3_SETMIN,'f',0)+"-");
    label134.setText(QString::number(((13.4-12)/(16-12))*(DE_HCO3_SETMAX-DE_HCO3_SETMIN)+DE_HCO3_SETMIN,'f',0)+"-");
    label125.setText(QString::number(((12.5-12)/(16-12))*(DE_HCO3_SETMAX-DE_HCO3_SETMIN)+DE_HCO3_SETMIN,'f',0)+"-");
...
//页面显示调用函数--#/
void SceneSetupHco3Curve::setCusVisible(bool vis, char *para)
{
    para=para;

#if SYSTEM_DEBUG_FLAG
    Debug_insert("SceneSetupHco3Curve::setCusVisible-in");
#endif
...
#if SYSTEM_DEBUG_FLAG
    Debug_insert("SceneSetupHco3Curve::setCusVisible-out");
#endif
}

void SceneSetupHco3Curve::ActionHandle(const void* dataPtr, int name)
{
#if SYSTEM_DEBUG_FLAG
    Debug_insert("SceneSetupHco3Curve-ActionHandle-in");
#endif
...
#if SYSTEM_DEBUG_FLAG
    Debug_insert("SceneSetupHco3Curve-ActionHandle-out");
#endif
}

//根据选择的曲线号，读取对应的数据文件显示曲线#/
void SceneSetupHco3Curve::selectedCurve(int num)
{
    num=num;
...
//编辑框选择时#/
void SceneSetupHco3Curve::lineEditInFocus()
{
...
//全加按钮点击时#/
void SceneSetupHco3Curve::allAddClicked()
{
...
//全减按钮点击#/
void SceneSetupHco3Curve::allSubClicked()
{
...
//曲线选择按钮---+#/
void SceneSetupHco3Curve::selectAddClicked()
{
    nowSelectCurve++;
...
void SceneSetupHco3Curve::selectSubClicked()
{
    nowSelectCurve--;
    if(nowSelectCurve<0)
        nowSelectCurve=15;
...
void SceneSetupHco3Curve::buttonSaveClicked()//保存按钮#/
{
...
void SceneSetupHco3Curve::buttonOkClicked()
{
    QString str;
    int i=0;

    if((residue_secCount<3600)&&(G_HCO3_PROFILE_FLAG==false))//小于60分钟不给应用#/
    {
    ...
    if(G_HCO3_PROFILE_FLAG==true)  //该曲线正在应用,是否取消#/
    {
        //确定是否应用#/
        ...
        else
        {
            ...
            formula.tempBuffer.hco3 = G_HCO3_PROFILE_RELVULE;
    ...
    else if(G_HCO3_PROFILE_FLAG==false) //该曲线未应用,是否应用#/
    {
        //确定是否应用#/
        ...
        G_HCO3_PROFILE_FLAG=true;

        //曲线应用处理#/
        for(i=0;i<10;i++)
        {
            A_HCO3_PROFILE[i].curevalue=curveArray[i]->naValue;
            A_HCO3_PROFILE[i].flag=false;
        }
        G_HCO3_PROFILE_NUM=0;      //曲线当前运行第几段#/
        G_HCO3_PROFILE_START=G_SYS_CURTIME;  //曲线开始时间#/
        G_HCO3_PROFILE_END=G_SYS_CURTIME+residue_secCount;    //曲线结束时间#/


        FormulaSet formula;
        dataRead(PACKET_FORMULA_TARGET,&formula);
        G_HCO3_PROFILE_RELVULE=formula.tempBuffer.hco3;    //曲线运行前的设置值#/
    }
...
//取消#/
void SceneSetupHco3Curve::buttonCancelClicked()
{
