...
/*-------------------------柱状图类----------------------------------*/
//界面设置函数#/
inline void ColumTempCurve::setUi()
{
...
//构造函数#/
ColumTempCurve::ColumTempCurve(QWidget *parent,SceneSetupTempCurve *p,int x,int y,int order):
...
void ColumTempCurve::mousePressEvent(QMouseEvent *ev)
{
    if(selectMark==0)
        return;
    ...
    naValue=usr_params.Params_Dialyser.Temp.attribute.max_value-((ev->y()-3)*((usr_params.Params_Dialyser.Temp.attribute.max_value-usr_params.Params_Dialyser.Temp.attribute.min_value)/(SCENE_SETUP_CURVE_STRIP_HIGH-6.0)));
...
//鼠标移动事件#/
void ColumTempCurve::mouseMoveEvent(QMouseEvent *ev)
{
    if(passMark==false)
        return;
...
//鼠标释放事件#/
void ColumTempCurve::mouseReleaseEvent(QMouseEvent *ev)
{
    passMark=false;
    ev->ignore();
...
//焦点进入#/
void ColumTempCurve::focusInEvent(QFocusEvent *ev)
{
...
//焦点移出#/
void ColumTempCurve::focusOutEvent(QFocusEvent *ev)
{
    ev=ev;
    selectMark=0;
    this->setStyleSheet("border-width: 3px; border-style: outset; \
...
//重绘事件#/
void ColumTempCurve::paintEvent(QPaintEvent *)
{
    QPainter paint(this);

    naValue=naValue>usr_params.Params_Dialyser.Temp.attribute.max_value?usr_params.Params_Dialyser.Temp.attribute.max_value:naValue;
    naValue=naValue<usr_params.Params_Dialyser.Temp.attribute.min_value?usr_params.Params_Dialyser.Temp.attribute.min_value:naValue;
    float high=((usr_params.Params_Dialyser.Temp.attribute.max_value-naValue)*1.0/(usr_params.Params_Dialyser.Temp.attribute.max_value-usr_params.Params_Dialyser.Temp.attribute.min_value))*(SCENE_SETUP_CURVE_STRIP_HIGH-6.0);
...
//  qDebug()<<"paintEvent";
    ...
    //显示#/
void ColumTempCurve::setShow(bool vis)
{
    this->setVisible(vis);
...
/*-------------------------曲线页面显示类-------------------------------*/
//界面设置函数#/
inline void SceneSetupTempCurve::setUi()
{
...
//构造函数#/
SceneSetupTempCurve::SceneSetupTempCurve(QWidget *parent):
QObject(parent),
...
//                      0.01,"Na Value:","Na",false,INT);
      ...
      ic->appendWidget(&lineEditSetNum,NumInputMethod,LineEdit,NULL,30,40,
                        0.1,"Temp Value:","Temp",false,INT);
 ...
 //   nowSelectCurve=0;
    for(int i=0;i<10;i++)
    {
        curveArray[i]=new ColumTempCurve(parent,this,SCENE_SETUP_CURVE_START_X+52*i,
                                        SCENE_SETUP_CURVE_START_Y,i);
        curveArray[i]->naValue=37.5;
...
SceneSetupTempCurve::~SceneSetupTempCurve()
{
    for(int i=0;i<10;i++)
    {
...
//界面文字显示设置#/
void SceneSetupTempCurve::CustomSetText()
{

    labelTitleTop.setText(DE_TR_TEMPPROFILE);
...
//    labelYaxisUnit.setText("[ms/cm]");
    labelYaxisUnit.setText("[C]");


    label160.setText(QString::number(usr_params.Params_Dialyser.Temp.attribute.max_value)+"-");
    label151.setText(QString::number(((15.1-12)/(16-12))*(usr_params.Params_Dialyser.Temp.attribute.max_value-usr_params.Params_Dialyser.Temp.attribute.min_value)+usr_params.Params_Dialyser.Temp.attribute.min_value,'f',0)+"-");
    label142.setText(QString::number(((14.2-12)/(16-12))*(usr_params.Params_Dialyser.Temp.attribute.max_value-usr_params.Params_Dialyser.Temp.attribute.min_value)+usr_params.Params_Dialyser.Temp.attribute.min_value,'f',0)+"-");
    label134.setText(QString::number(((13.4-12)/(16-12))*(usr_params.Params_Dialyser.Temp.attribute.max_value-usr_params.Params_Dialyser.Temp.attribute.min_value)+usr_params.Params_Dialyser.Temp.attribute.min_value,'f',0)+"-");
    label125.setText(QString::number(((12.5-12)/(16-12))*(usr_params.Params_Dialyser.Temp.attribute.max_value-usr_params.Params_Dialyser.Temp.attribute.min_value)+usr_params.Params_Dialyser.Temp.attribute.min_value,'f',0)+"-");
...
//页面显示调用函数--#/
void SceneSetupTempCurve::setCusVisible(bool vis, char *para)
{
    para=para;

#if SYSTEM_DEBUG_FLAG
    Debug_insert("SceneSetupTempCurve::setCusVisible-in");
#endif
...
#if SYSTEM_DEBUG_FLAG
    Debug_insert("SceneSetupTempCurve::setCusVisible-out");
#endif
}

void SceneSetupTempCurve::ActionHandle(const void* dataPtr, int name)
{
#if SYSTEM_DEBUG_FLAG
    Debug_insert("SceneSetupTempCurve-ActionHandle-in");
#endif
...
#if SYSTEM_DEBUG_FLAG
    Debug_insert("SceneSetupTempCurve-ActionHandle-out");
#endif
}

//根据选择的曲线号，读取对应的数据文件显示曲线#/
void SceneSetupTempCurve::selectedCurve(int num)
{
    num=num;
...
//编辑框选择时#/
void SceneSetupTempCurve::lineEditInFocus()
{
...
//全加按钮点击时#/
void SceneSetupTempCurve::allAddClicked()
{
...
//全减按钮点击#/
void SceneSetupTempCurve::allSubClicked()
{
...
//曲线选择按钮---+#/
void SceneSetupTempCurve::selectAddClicked()
{
    nowSelectCurve++;
...
void SceneSetupTempCurve::selectSubClicked()
{
    nowSelectCurve--;
    if(nowSelectCurve<0)
        nowSelectCurve=15;
...
void SceneSetupTempCurve::buttonSaveClicked()//保存按钮#/
{
...
void SceneSetupTempCurve::buttonOkClicked()
{
    QString str;
    int i=0;

    if((residue_secCount<3600)&&(G_TEMP_PROFILE_FLAG==false))//小于60分钟不给应用#/
    {
    ...
    if(G_TEMP_PROFILE_FLAG==true)  //该曲线正在应用,是否取消#/
    {
        //确定是否应用#/
        ...
        else
        {
            ...
            packet.tempTarget = G_TEMP_PROFILE_RELVULE;
    ...
    else if(G_TEMP_PROFILE_FLAG==false) //该曲线未应用,是否应用#/
    {
        //确定是否应用#/
        ...
        G_TEMP_PROFILE_FLAG=true;

        //曲线应用处理#/
        for(i=0;i<10;i++)
        {
            A_TEMP_PROFILE[i].curevalue=curveArray[i]->naValue;
            A_TEMP_PROFILE[i].flag=false;
        }
        G_TEMP_PROFILE_NUM=0;      //曲线当前运行第几段#/
        G_TEMP_PROFILE_START=G_SYS_CURTIME;  //曲线开始时间#/
        G_TEMP_PROFILE_END=G_SYS_CURTIME+residue_secCount;    //曲线结束时间#/
        ...
        G_TEMP_PROFILE_RELVULE=packet.tempTarget;    //曲线运行前的设置值#/
    }
...
//取消#/
void SceneSetupTempCurve::buttonCancelClicked()
{
