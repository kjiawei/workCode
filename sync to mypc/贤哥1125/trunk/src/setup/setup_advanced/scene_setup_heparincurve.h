#ifndef SCENE_SETUP_HEPARINCURVE_H
#define SCENE_SETUP_HEPARINCURVE_H
...
class SceneSetupHeparinCurve;



/*-------------------------柱状图类-------------------------------*/
//每个治疗时段的柱状图类#/
class ColumHeparinCurve: public QLabel
{
    Q_OBJECT

public:
    ColumHeparinCurve(QWidget *parent,SceneSetupHeparinCurve *p,int x,int y,int order);
...
private:
    ...
    int yAxis;//y坐标#/
    bool passMark; //点击标记#/
    SceneSetupHeparinCurve *topscene; //为了操控上层页面的控件而定义#/
    QLabel labeltop;  //柱状图顶部显示数据 超滤率=平均超滤量*百分比 [ml/h]#/
};

/*-------------------------钠曲线显示编辑页面类-------------------------------*/
//钠曲线显示编辑页面类#/
class SceneSetupHeparinCurve:public QObject, public ActionCustom,public PieceVisible,public LangSetText
{
    Q_OBJECT
public:
    SceneSetupHeparinCurve(QWidget *parent=0); //构造函数#/
    ~SceneSetupHeparinCurve();
...
public:
    ...
    ColumHeparinCurve *curveArray[10]; //指向柱状图的指针，默认数量是10个#/
    //各种文本标签#/
