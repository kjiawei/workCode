#ifndef SCENE_SETUP_CONDCURVE_H
#define SCENE_SETUP_CONDCURVE_H
...
class SceneSetupCondCurve;



/*-------------------------柱状图类-------------------------------*/
//每个治疗时段的柱状图类#/
class ColumCondCurve: public QLabel
{
    Q_OBJECT

public:
    ColumCondCurve(QWidget *parent,SceneSetupCondCurve *p,int x,int y,int order);
...
private:
    ...
    int yAxis;//y坐标#/
    bool passMark; //点击标记#/
    SceneSetupCondCurve *topscene; //为了操控上层页面的控件而定义#/
    QLabel labeltop;  //柱状图顶部显示数据 超滤率=平均超滤量*百分比 [ml/h]#/
};

/*-------------------------钠曲线显示编辑页面类-------------------------------*/
//钠曲线显示编辑页面类#/
class SceneSetupCondCurve:public QObject, public ActionCustom,public PieceVisible,public LangSetText
{
    Q_OBJECT
public:
    SceneSetupCondCurve(QWidget *parent=0); //构造函数#/
    ~SceneSetupCondCurve();
...
public:
    ...
    ColumCondCurve *curveArray[10]; //指向柱状图的指针，默认数量是10个#/
    //各种文本标签#/
