...
void SceneKTV2::setUi(void)
{
    ...
    labelAttentionText->setStyleSheet(CONFIG_COLOR_LABEL_RED);
    ...
    ureaExcRatio->setGeometry(0,160,200,38);//KTV标签左上角即为0点
    ...
    ureaExcRatioUnit = new QLabel(DialyzerP);//单位
    SCENE_KTV_LABLE(*ureaExcRatioUnit, SCENE_PATIENT_LABEL_X + 40, 3, 200);
    ureaExcRatioUnit2 = new QLabel(DialyzerP);//单位
    SCENE_KTV_LABLE(*ureaExcRatioUnit2, SCENE_PATIENT_LABEL_X + 40, 4, 200);

    ureaExc = new QLineEdit(DialyzerP);//编辑框
    ureaExc2 = new QLineEdit(DialyzerP);
    SCENE_KTV_LINEEDIT(*ureaExc, 250, 3);
    SCENE_KTV_LINEEDIT(*ureaExc2, 250, 4);//SCENE_PATIENT_LINEEDIT_X

    defaultUreaExc = new QLabel(DialyzerP);//默认值
    defaultUreaExc2 = new QLabel(DialyzerP);
    SCENE_KTV_LABLE(*defaultUreaExc, 350, 3, 200);
    SCENE_KTV_LABLE(*defaultUreaExc2, 350, 4, 200);

    labelKTV = new QLabel(DialyzerP);
    labelKTV->setGeometry(0,260,200,38);//260->325
    ...
    ktvValue = new QLabel(DialyzerP);
    ktvValue->setGeometry(SCENE_PATIENT_LABEL_X + 120,260,200,38);
    ktvValue->setAlignment( Qt::AlignVCenter | Qt::AlignLeft);
    ktvValue->setFont(QFont(QString(CONFIG_FONT_STYLE),
                                 CFG_FONT_SIZE_THIRD, CONFIG_FONT_NORMAL));
    ktvValue->setStyleSheet(CONFIG_COLOR_WHITE_CHARS);

    treatTime = new QLabel(DialyzerP);
    treatTime->setGeometry(260,260,200,38);
    treatTime->setAlignment( Qt::AlignVCenter | Qt::AlignLeft);
    treatTime->setFont(QFont(QString(CONFIG_FONT_STYLE),
                                  CFG_FONT_SIZE_THIRD, CONFIG_FONT_NORMAL));
    treatTime->setStyleSheet(CONFIG_COLOR_WHITE_CHARS);

    treatTimeUnit = new QLabel(DialyzerP);
    treatTimeUnit->setGeometry(500,260,200,38);
    treatTimeUnit->setAlignment( Qt::AlignVCenter | Qt::AlignLeft);
    treatTimeUnit->setFont(QFont(QString(CONFIG_FONT_STYLE),
                                      CFG_FONT_SIZE_THIRD, CONFIG_FONT_NORMAL));
    treatTimeUnit->setStyleSheet(CONFIG_COLOR_WHITE_CHARS);

    treatTimeEdit = new QLineEdit(DialyzerP);
    SCENE_KTV_LINEEDIT(*treatTimeEdit, 380, 6-20);

    adpPro = new QLabel(DialyzerP);//调整比例
    adpPro->setGeometry(0,320,200,38);//KTV标签左上角即为0点
    adpPro->setAlignment( Qt::AlignVCenter | Qt::AlignLeft);
    adpPro->setFont(QFont(QString(CONFIG_FONT_STYLE),
                                           CFG_FONT_SIZE_THIRD, CONFIG_FONT_NORMAL));
    adpPro->setStyleSheet(CONFIG_COLOR_WHITE_CHARS);

    adpProDef = new QLabel(DialyzerP);
    adpProDef->setGeometry(250,320,200,38);//KTV标签左上角即为0点
    adpProDef->setAlignment( Qt::AlignVCenter | Qt::AlignLeft);
    adpProDef->setFont(QFont(QString(CONFIG_FONT_STYLE),
                                                  CFG_FONT_SIZE_THIRD, CONFIG_FONT_NORMAL));
    adpProDef->setStyleSheet(CONFIG_COLOR_WHITE_CHARS);

    adpProDefV = new QLabel(DialyzerP);
    adpProDefV->setGeometry(380,320,200,38);//KTV标签左上角即为0点
    adpProDefV->setAlignment( Qt::AlignVCenter | Qt::AlignLeft);
    adpProDefV->setFont(QFont(QString(CONFIG_FONT_STYLE),
                                                       CFG_FONT_SIZE_THIRD, CONFIG_FONT_NORMAL));
    adpProDefV->setStyleSheet(CONFIG_COLOR_WHITE_CHARS);

    adpProEdit = new QLineEdit(DialyzerP);
    SCENE_KTV_LINEEDIT(*adpProEdit, SCENE_PATIENT_LABEL_X + 120, 7);
    /***********************************Curve Page************************************/
...
void SceneKTV2::CustomSetText()
{
    ...
    //DialysisP
    ureaExcRatio->setText(tr("CarbamideClearAmount"));
    ...
    adpPro->setText(tr("AdaptiveProportion"));
