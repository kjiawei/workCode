...
audioServer::audioServer(void):audio_sem(0)
{
    flag = 1;
    alarmBit = false;//true;
	//audio_sem=0;
    ...
    else
    {
        ...
        start(QThread::TimeCriticalPriority);//开始一个线程，线程任务通过调用虚函数run实现
    }
#endif
}
...
int audioserver::initDSP(QString filename)
{
    ...
    fwave = fopen(filename.toLatin1().data(), "r");//打开音频文件
    if(fwave == NULL) {
...
void audioServer::run()//声音告警线程任务内容处理
{
#ifdef OS_LINUX
    int readbytes = 0;
    int writebytes = 0;
    ...
    Debug_insert("audioserver::run_in");
            ...
            if(G_sound_shielding==true)      //工程模式声音静音标志#/
            {
                ...
                { //当播放的长度达到要求长度时，即音频播放完毕时#/
                    ...
                    //判断如是报警音，则延时后，重复播放#/
                    if(alarmBit == true && alarm_filename != "" && isAlarmSilent==false)
                    {
                        flag = 4;
                        msleep(this->msDelay);
                        saveflag=0;
                ...
                {
                    if(alarmBit == true && isAlarmSilent==false) {
